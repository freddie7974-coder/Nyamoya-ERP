...
export default function BalanceSheetScreen({ onBack }) {
  ...
  // 1. Fetch Inventory Value Automatically 🕵️‍♂️
  useEffect(() => {
  ...
  const calculateInventoryValue = async () => {
    try {
      ...
      prodSnap.forEach(doc => {
        const data = doc.data()
        // Value = Quantity * Cost Per Unit (Assuming standard selling price or cost if you have it. Using simple logic here)
        // Note: Ideally you use Cost Price, but if not tracked, we estimate.
        // Let's assume a rough 60% of selling price is cost if exact cost isn't stored, OR use data.costPerUnit if you added it previously.
        const cost = data.costPerUnit || (data.price * 0.6) || 0 
    ...
    <Box p={4} maxW="900px" mx="auto">
      ...
      <Box textAlign="center" p={4} bg={TOTAL_ASSETS === (TOTAL_LIABILITIES + TOTAL_EQUITY) ? "teal.50" : "red.100"} borderRadius="xl">
        ...
        {TOTAL_ASSETS === (TOTAL_LIABILITIES + TOTAL_EQUITY) ? (
          ...
          <Text color="red.600">❌ Calculation Error (Should be impossible with this logic)</Text>
        )}
      </Box>
    </Box>
  )
